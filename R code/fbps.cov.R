fbps.cov <-function(data,x = NULL, diag.remove = TRUE, knots=35, p=3,m=2,lambda=NULL,search.grid = T, search.length = 20, method="L-BFGS-B",lower= -20, upper=20, control=NULL){
...
#source("/Users/xiaoyuesixi/Desktop/fbps_2012/fbps.code.2012/pspline.setting.R")
## make sure the data is symmetric
data = (data+t(data))/2
n = dim(data)[1] 

if(is.null(x)) x=(1:n)/n-1/2/n ## if NULL, assume equally distributed 
...
lambda=exp(x)
sigma = kronecker(1/(1+lambda*s),1/(1+lambda*s))
sigma.2 = sqrt(sigma)
...
Temp = Sigi_sqrt%*%U%*%diag(1/(1+lambda*s))
Theta = Temp%*%Ytilde%*%t(Temp)
hatY = B%*%Theta%*%t(B)
...
if(search.grid){
    ...
    GCV = rep(0,lambda.length)
    for(j in 1:lambda.length)
        GCV[j] = fbps_cov_gcv(Lambda[j])
      
    i0 = which.min(GCV)
    temp.lambda = GCV[i0]
    i1 = i0
    while(GCV[i1]<=1.05*temp.lambda&&i1<lambda.length) i1 = i1 + 1
...
if(!diag.remove) var.noise = NULL

result=list(cov = fbps_cov, var = var.noise,lambda = lambda)
# result = list(Theta = est$Theta,lambda=lambda)
return(result)
}
