  ...
  for(L in seqL){
    ...
    for(i_fold in 1:kfold){
      ...
      if(L==0){
        ...
        sigma2_hat <- rep(NA, J)
        for(j in 1:J){
          face_fit <- fpca.face(Y=Y_train[[j]], argvals=argvals, var=T)
          sigma2_hat[j] <- face_fit$sigma2
        ...
        est[[(which(seqL==L)-1)*kfold+i_fold]] = list(C=NULL, beta=NULL, G=G_hat, sigma2=sigma2_hat)
        ###################################################################################
